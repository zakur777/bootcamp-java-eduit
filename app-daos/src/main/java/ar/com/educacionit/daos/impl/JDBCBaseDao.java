...
public abstract class JDBCBaseDao<T extends Entity> implements GenericDao<T> {
	...
	public JDBCBaseDao(String tabla) {
		if (tabla == null) {
			throw new IllegalArgumentException("Debe iniciar la tabla del DAO");
		}
		this.tabla = tabla;
	}

	// cada implementacion debe especificarlo
	public abstract String getSaveSQL();

	public abstract void saveData(T entity, PreparedStatement st) throws SQLException;
	...
	public T getByPK(Long id) throws GenericException {
		try (Connection con2 = AdministradorDeConexiones.obtenerConexion()) {
			try (Statement st = con2.createStatement()) {
				...
				throw new GenericException("No se pudo obtener el entety " + this.tabla + " id:" + id, e);
			}
		} catch (SQLException e) {
	...
	// cada hijo convierte el ResultSet a la entidad que corresponde
